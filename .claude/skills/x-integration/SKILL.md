---
name: x-integration
description: X (Twitter) integration for PaperClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** PaperClaw v1.0.0. Directory structure may change in future versions.
...
1. **PaperClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep paperclaw (macOS) or systemctl --user status paperclaw (Linux)
```
...
| `PAPERCLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/paperclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into PaperClaw, make the following modifications:
...
All paths below are relative to project root (`PAPERCLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep paperclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status paperclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/paperclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/paperclaw.log | tail -20
```
...
docker run paperclaw-agent ls -la /app/src/skills/
```
