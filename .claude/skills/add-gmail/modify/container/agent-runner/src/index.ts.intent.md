...
- Added: `gmail` MCP server alongside the existing `paperclaw` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `paperclaw` MCP server configuration is unchanged
...
## Must-keep
- The `paperclaw` MCP server with its environment variables
