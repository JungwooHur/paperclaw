...
Adds Parallel AI MCP integration to PaperClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses PaperClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. PaperClaw already set up and running
...
Add Parallel AI MCP servers after the paperclaw server:
```typescript
const mcpServers: Record<string, any> = {
  paperclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__paperclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__paperclaw__send_message
   4. Use mcp__paperclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo paperclaw-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/paperclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.paperclaw` (macOS) or `systemctl --user restart paperclaw` (Linux)
