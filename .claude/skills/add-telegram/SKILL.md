...
This skill adds Telegram support to PaperClaw using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.paperclaw/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep paperclaw` (macOS) or `systemctl --user status paperclaw` (Linux)
...
- Check bot is started: `tail -f logs/paperclaw.log`
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.paperclaw` (macOS) or `npm run build && systemctl --user restart paperclaw` (Linux)
