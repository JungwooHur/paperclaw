---
name: setup
description: Run initial PaperClaw setup. Use when user wants to install dependencies, authenticate WhatsApp, register their main channel, or start the background services. Triggers on "setup", "install", "configure paperclaw", or first-time setup requests.
---
...
- macOS: `launchctl unload ~/Library/LaunchAgents/com.paperclaw.plist`
- Linux: `systemctl --user stop paperclaw` (or `systemctl stop paperclaw` if root)

Run `npx tsx setup/index.ts --step service` and parse the status block.

**If FALLBACK=wsl_no_systemd:** WSL without systemd detected. Tell user they can either enable systemd in WSL (`echo -e "[boot]\nsystemd=true" | sudo tee /etc/wsl.conf` then restart WSL) or use the generated `start-paperclaw.sh` wrapper.
...
- macOS: check `launchctl list | grep paperclaw`. If PID=`-` and status non-zero, read `logs/paperclaw.error.log`.
- Linux: check `systemctl --user status paperclaw`.
...
- SERVICE=stopped → `npm run build`, then restart: `launchctl kickstart -k gui/$(id -u)/com.paperclaw` (macOS) or `systemctl --user restart paperclaw` (Linux) or `bash start-paperclaw.sh` (WSL nohup)
...
Tell user to test: send a message in their registered chat. Show: `tail -f logs/paperclaw.log`

## Troubleshooting

**Service not starting:** Check `logs/paperclaw.error.log`. Common: wrong Node path (re-run step 10), missing `.env` (step 4), missing auth (step 5).

**Container agent fails ("Claude Code process exited with code 1"):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.

**No response to messages:** Check trigger pattern. Main channel doesn't need prefix. Check DB: `npx tsx setup/index.ts --step verify`. Check `logs/paperclaw.log`.

**WhatsApp disconnected:** `npm run auth` then rebuild and restart: `npm run build && launchctl kickstart -k gui/$(id -u)/com.paperclaw` (macOS) or `systemctl --user restart paperclaw` (Linux).

**Unload service:** macOS: `launchctl unload ~/Library/LaunchAgents/com.paperclaw.plist` | Linux: `systemctl --user stop paperclaw`
