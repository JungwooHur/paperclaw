 ...
 *   2. Initialize .paperclaw/ (snapshot current source as base)
...
function resetWorkingTree(): void {
  ...
  if (fs.existsSync('.paperclaw')) {
    fs.rmSync('.paperclaw', { recursive: true, force: true });
  }
}

function initPaperclaw(): void {
  execSync(
    'npx tsx -e "import { initPaperclawDir } from \'./skills-engine/index\'; initPaperclawDir();"',
