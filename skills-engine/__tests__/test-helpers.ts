...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'paperclaw-test-'));
}

export function setupPaperclawDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.paperclaw', 'base', 'src'), {
    recursive: true,
  });
  fs.mkdirSync(path.join(tmpDir, '.paperclaw', 'backup'), { recursive: true });
}

export function writeState(tmpDir: string, state: any): void {
  const statePath = path.join(tmpDir, '.paperclaw', 'state.yaml');
