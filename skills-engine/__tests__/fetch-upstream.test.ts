...
describe('fetch-upstream.sh', () => {
  ...
  beforeEach(() => {
    ...
    upstreamBareDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'paperclaw-upstream-'),
    );
    ...
    const seedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'paperclaw-seed-'));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'paperclaw', version: '2.0.0' }),
    );
    ...
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'paperclaw-project-'));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'paperclaw', version: '1.0.0' }),
    );
  ...
  it('uses existing upstream remote', () => {
    ...
    expect(status.TEMP_DIR).toMatch(/^\/tmp\/paperclaw-update-/);
