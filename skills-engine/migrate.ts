...
import { BASE_DIR, CUSTOM_DIR, PAPERCLAW_DIR } from './constants.js';
import { initPaperclawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initPaperclawDir();
  console.log('Skills system initialized. .paperclaw/ directory created.');
}
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      const filesModified = [...diff.matchAll(/^diff -ruN .+ (.+)$/gm)]
        ...
        .filter((f) => !f.startsWith('.paperclaw'));
      ...
      console.log(
        'Custom modifications captured in .paperclaw/custom/migration.patch',
      );
    } else {
