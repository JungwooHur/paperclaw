/**
 * Container Runner for PaperClaw
...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // NotebookLM state — writable so the container can create notebooks,
    // save conversation IDs, and update session state. Auth cookies
    // (storage_state.json) are only modified by `notebooklm login` which
    // the container never calls, so the login session stays intact.
    const notebooklmHome = path.join(os.homedir(), '.notebooklm');
    if (fs.existsSync(notebooklmHome)) {
      mounts.push({
        hostPath: notebooklmHome,
        containerPath: '/home/node/.notebooklm',
        readonly: false,
      });
    }
  } else {
...
export async function runContainerAgent(
  ...
  const containerName = `paperclaw-${safeName}-${Date.now()}`;
