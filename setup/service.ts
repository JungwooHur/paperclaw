...
<dict>
    ...
    <string>${projectRoot}/logs/paperclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/paperclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.paperclaw');
  } catch {
...
/**
 * Kill any orphaned paperclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned paperclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/paperclaw.service';
    ...
    unitPath = path.join(unitDir, 'paperclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=PaperClaw Personal Assistant
...
StandardError=append:${projectRoot}/logs/paperclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned paperclaw processes to avoid WhatsApp conflict errors
  ...
  try {
    execSync(`${systemctlPrefix} enable paperclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start paperclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active paperclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-paperclaw.sh');
  const pidFile = path.join(projectRoot, 'paperclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-paperclaw.sh — Start PaperClaw without systemd',
    ...
    '    echo "Stopping existing PaperClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting PaperClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/paperclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/paperclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "PaperClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/paperclaw.log"`,
  ];
