...
> onto the reader. Answer, then save. (A pair where the bot offered instead of saving
> is treated by `auto_save_qa.py` as proof the save did not happen, so the backstop
> files it within a cycle — but the user still saw the question, so don't ask.)
