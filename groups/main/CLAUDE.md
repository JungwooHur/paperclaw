...
## Repairs that are not on the healer

Three repairs are deliberately NOT wired into the five-minute cycle, and each
has its own reason:

```bash
cd /workspace/group/research-papers
# a Methods section that came through as one block (run-in headings)
python3 paragraph_headings.py --page <page_id> --apply
# citations glued to the word before them, from a superscript style
python3 superscript_citations.py --page <page_id> --max-ref <N> --apply
# a bibliography for a paper published outside the arXiv ecosystem
python3 nature_source.py --page <page_id> --url <article_url> --apply
```

Each prints what it WOULD do when `--apply` is omitted, and each refuses rather
than guesses. Why they stay manual:

* The citation repair **edits body text**. It only ever inserts, and it checks
  the invariant against the real text before every write — but putting an editor
  of somebody's reading material on a timer is a different decision from writing
  one, and this project has already paid for a repair that ran unattended and
  rewrote the same block for weeks.
* The bibliography injector maps `[7]` to entry 7 with **nothing to verify it
  against** — a publisher page carries no in-body anchors. That holds when the
  numbering is the paper's own, which a person confirms from a sample; a timer
  cannot.
* The run-in splitter reads a convention that **differs between journals**. It
  is the safest of the three, and it is kept here so all three are found
  together rather than one drifting off on its own.

**When a page needs one of these, it needs a person.** Nothing surfaces that
automatically today, which is the honest state: these are ungated procedures,
and the gate that exists only checks that the commands above are real.
...
| Area | Root Cause | Fix |
|------|-----------|-----|
| A processing request filed onto a paper as a Q&A | The writer every Q&A must go through validated nothing, so the rule that it be used protected nothing — whatever the agent passed was written, under a heading of its own. The judgement that would have caught it lived only in the backstop, and it was fooled anyway: it strips imperative endings, but the attachment path a message arrives with pushed a bare request past the length that the fallback reads as "long enough to be a question" | `question_shape` holds the judgement for both callers and removes the attachment line before judging. `save_qa_callout` refuses anything that is not a question — the `Q: ` prefix the caller adds is not evidence, what is asked after it is |
| A journal paper gets no reference list at all | The citation pipeline reads LaTeXML HTML, so a paper published outside that ecosystem has nothing to work with: the page ends up with a body full of markers pointing nowhere. Such publishers commonly expose the bibliography even where the full text is not | `nature_source` reads the list from the article page and reuses the existing injector and linker. **Run by hand** — see the section on repairs that are not on the healer — because the numbering cannot be verified. **There is no source body to align against**, so the mapping is the identity one — `[7]` means entry 7 — which holds when the page's numbering is the paper's own. Check a sample by hand before trusting a new source; the injected list makes that possible by eye |
| Citations glued to the word before them, and unlinkable | A journal that sets citations as superscripts loses them to a translation made from the rendered paper: the body reads `Method11,12`, the number looks like part of the word, and only bracketed markers are recognised | `superscript_citations` brackets them, **run by hand** — see the section on repairs that are not on the healer — because it edits body text, which is not something to put on a five-minute timer. It INSERTS and never replaces — stripping brackets and spaces returns the original, checked against the real text before each write — and refuses anything ambiguous: a number beyond the bibliography, a year, a measurement like `19x19`, a decimal, an identifier ending in digits. **The lookahead excludes a Latin letter, not any word character**: a Korean particle glues straight onto a citation, and excluding word characters in general made every such citation invisible — the same trap as `\b` before a particle, for the fourth time |
| A Methods section arrives as one undifferentiated block | Journals run their Methods headings into the paragraph — bold in the original, plain words after translation — so fifteen thousand characters can sit under a single heading. What makes them recognisable is the translation's own convention: a heading is written `English Title (한글 제목)` and is followed by a space, while a term gloss in running prose has a Korean particle glued onto the bracket | `paragraph_headings.split_run_in`, **run by hand** — see the section on repairs that are not on the healer. A title must be plain text: an equation reaching into it means the phrase is part of a sentence, not a heading. **Slicing a span means slicing characters, and an equation has none** — writing text into one is rejected by Notion with a 400, which is how four blocks of a page were split before the rest failed |
| A page updated itself every five minutes for weeks, with nothing visibly changing | `clean_source_urls` decided it had changed a run the moment the run LOOKED like it needed cleaning — `changed = True` sat outside any comparison — while the cleaner itself only strips real URLs. The detector matches bare words, so it fires on prose that merely NAMES a source: a paper listing its training data writes "arXiv.org" in a sentence. So the block was rewritten with byte-identical content, forever. The text never moved, which is why nobody saw it; but every write bumped `last_edited_time`, which kept the page inside the healer's 3-hour window, which kept EVERY other healer re-running on it too | A repair reports a change only when the content actually differs. Gated by `tests/test_cleaner_converges.py`, including that a second pass over the first pass's output finds nothing. **Any repair here must be idempotent: running it twice has to equal running it once, or the five-minute healer never converges and the page never leaves the window.** The same shape exists in `strip_furniture`; it cannot currently misfire (its pattern always replaces a phrase with one space) and a test pins that down |
| A fully translated page reported as "nothing translated yet", and every structural check skipping it | Section titles came out as ordinary PARAGRAPHS, so the page had no headings — and the audit's no-headings branch reported `NOT_TRANSLATED`, which is the NORMAL state of a page just added to the DB and is therefore reported quietly. One page sat that way holding 40,000 characters of translation and a duplicated half: the audit groups sections by heading, the duplicate healer keys on heading, and neither could see anything at all | `paragraph_headings.no_heading_finding` now tells the three states apart — waiting for translation, figures with no text (a skipped translation), and titles-as-paragraphs — and the last is flagged LOUDLY. `heal_verify` promotes those titles to headings first and then RE-AUDITS, because the audit that ran before them could not have reported a duplicate it had no sections to see. On that page the promotion immediately surfaced 11 DUPLICATE and 47 PARA_DUP findings |
| A transient Notion 429 killed a whole healer cycle | Every script reaches Notion through two paths. The write path in `translate_fulltext.notion` retried; the READ path (`auto_save_qa.api_get`, which `verify_sections.fetch_blocks` uses) did not, so one rate limit during a burst aborted whatever was running — an audit lost its page, an injection stopped mid-section | Both paths now share `notion_retry`: rate limits and 5xx are retried, 400/404 are not, Notion's own `Retry-After` decides the wait, and the wait is capped so a retry cannot become a hang. One module rather than a copy each, because two retry policies drift and the one that drifts is the one nobody watches |
| One question saved to its page two, three, four times | Two causes. (1) The agent saves its OWN Q&A — rephrased and usually much shorter than the reply it sends — so the backstop's wording dedup could not recognise it. Measured on real pages, wording cannot separate the cases at any threshold: a known duplicate's bodies overlap 0.19 while two genuinely different Q&As on one page reach 0.26. (2) The backstop is started from two places — the healer timer and the post-reply trigger — and only the trigger held a lock, so two scans ran at once, each concluded "not saved", and each saved | `backstop_timing.saved_during_exchange`: a callout created between the question and ten minutes after the reply, whose question overlaps the user's by ≥0.40, is the agent's save and the pair is skipped. Neither signal is used alone — timing alone would bury a forgotten Q2 under the Q1 the agent did save in the same window, and 0.40 alone is too loose (different questions reach 0.82 at the extreme). The lock moved INTO the script (`run_lock`, non-blocking — a second run exits and leaves the work to the first), so every caller is serialised whoever starts it; an unwritable lock path runs unlocked rather than never |
| A page grew to thousands of blocks — the same bibliography over and over | The injector refused to append only when it RECOGNISED its own list: the `References` heading AND a body that parses as entries. Recognition is the fragile half. This paper labels entries `[Agrawal et al. (2016)]`, whose dots, parentheses and ampersands the label pattern did not allow, so the tool could not see the list it had just written and the five-minute healer appended a fresh copy every cycle. One page reached 37 copies of its bibliography, 2,820 blocks, and no body at all | Two layers. The label pattern now accepts any short bracketed label carrying a digit (plus a single short word for a corporate tag), so number, author-initial and author-year styles are all recognised. And the decision "may I append a list?" is made on the HEADING ALONE (`link_references.may_inject_references`) — it never parses the entries, so no future label style can bring the runaway back. Proven by breaking recognition deliberately and watching the guard still refuse |
| A freshly translated paper has most of its citations missing | The translation source is NotebookLM's own indexed text, not the published HTML, and it leaves `\cite{...}` unresolved — the body reads `[ author2024method, shortkey]` where the HTML reads `[3, 7]`. Worse, it carries only part of them: measured 2026-09-03 on one paper, 44 citation groups in the indexed text against 73 anchors in the HTML. A translation cannot restore what its input never had | The linker accepts key-form citations and renumbers them like any other, so the ones that DO survive are linked. The rest is an input-side limit, not a translation or linking bug — do not go looking for one. `link_page` reports `warning: body has no citations (N in source)` for the extreme case |
| A figure the body never cites lands in the appendix | Its anchor is the first mention of its number, and a figure the text never cites — a teaser, or one whose citation the translation dropped — has none, so it fell to the page end, which IS the appendix. The backward fill added earlier only looked at LOWER-numbered figures, so a teaser numbered 1 had nothing to fall back on and went there every time. And when a run injects only the MISSING numbers, the figures already on the page were not candidates at all, so a single uncited figure had no neighbour to sit beside | `extract_paper_figures.fill_anchor_gaps` joins the nearest placed figure before it, and the nearest one AFTER when nothing is lower. `neighbour_spot` does the same against figures already on the page. Both are shared by the HTML and PDF injectors so the two cannot drift. Gated by `tests/test_anchor_gaps.py` |
| One dense paragraph cost a whole page its citation links | Notion allows 100 rich_text pieces per block and linking one citation costs several, so a related-work paragraph citing ~50 works cannot have them all linked however the spans are merged. Raising on it aborted the run: every OTHER paragraph lost its links too, over one paragraph nobody could have linked anyway | `_rewrite_block` links as many as fit, earliest first, and leaves the rest as the plain text they already were. Gated by `tests/test_span_limit.py`, including that the text is unchanged |
| `slots_linked: 0` that is not the linker's fault | On one paper the translation had dropped EVERY citation marker — 73 in the source, zero in the body — so there was nothing to link and no alignment work would have helped. The report looked identical to a linker that failed, and the fixes are opposite: a code change versus a re-translation | `link_page` now reports `warning: body has no citations (N in source)`. **A page in that state needs re-translating; the linker cannot help it** |
...
| A question naming its paper by a MODEL NUMBER is still filed under an unrelated one | The acronym tier fixed the all-caps case and stopped there, so a paper known by a model number — the Greek-lettered ones — still offered exactly ONE distinctive token and still lost the "≥2 distinct keywords" bar. Document frequency is no substitute for the naming test either: `fast` occurs in **14** titles, yet only one paper is NAMED FAST. And two independent traps sat behind it — the DB holds **duplicate pages** for these papers (`df['pi0.5'] == 2`, `df['pi0'] == 3`), one of them a collector stub under a shortened title, so a strict "shared name ⇒ ambiguous" rule threw the evidence away | The paper's NAME is the token its title opens with before the colon — an all-caps acronym or a token carrying a digit (latinized first, so `π0.5:` reads as `pi0.5:`). When several pages own a name, the one the reader means is **the one that HAS the paper on it**: prefer the owner with body text, and only call it ambiguous when more than one owner carries content. Verified on three real questions that had each been misfiled |
| A question naming its paper as `FAST논문에서` is filed under a paper whose title merely contains the word "fast" | Two things, both familiar. (1) **`\b` does not hold between a latin word and a Korean particle** — in `FAST논문에서` both `T` and `논` are word characters, so `\bFAST\b` never matches and the name the user typed is invisible. Third time this repo has hit that trap (section labels, figure anchors, now keyword attribution). (2) Even once it matches, an acronym is the ONLY distinctive token such a question has, so it cannot clear the "≥2 distinct keywords" bar — while an unrelated paper clears it on two generic words | Match keywords with `(?<![A-Za-z0-9])kw(?![A-Za-z0-9])`, which rejects only a longer token in the SAME script (verified: `FAST논문` matches, `breakfast논문` does not). And add a tier ABOVE the keyword bar: a title opening with an all-caps acronym carries the name people actually use, and it is title-unique in practice — of 57 such papers in the live DB only one acronym is shared, and a shared one is skipped as ambiguous. Case-SENSITIVE, because the capitals are what make it a name and a lowercase "fast" is just a word |
| A question that NAMES its paper is filed on a completely unrelated one | Two independent faults, and the first hides the second. (1) A model named with a **Greek letter** is written one way in the title and another by the person asking: the page says `\u03c00`, the question says `pi0`, and nobody types \u03c0 on a phone. `extract_title_keywords` requires a 4+ character ASCII/Hangul token, so the one word that identifies the paper was dropped and scoring fell back to generic title words. Subscript forms (`\u03c0\u2080.\u2087`) and a star (`\u03c0*0.6`) break it the same way. (2) Tier 1 ranked the user's question and the BOT's reply together — and an answer is the wrong thing to score, because it is a technical explanation whose vocabulary matches any paper sharing a rare word with the topic. The explanation of a method scored 1.07 on words like "interpolation" and "smoothing"; the paper the user actually named scored 0.64. **The Q&A was saved — onto the wrong paper's page**, which from the reader's side is indistinguishable from "it wasn't saved" | `latinize()` spells Greek letters out and folds subscript/superscript digits, applied to BOTH the title and the text being matched; a 3-character token is kept when it carries a DIGIT (`pi0`, `rt2`, `gr2` are model names, as title-unique as a token gets, while a 3-letter English word is the opposite). And Tier 1 now asks the USER's question first, falling back to the reply only when the question yields nothing — the rule Tier 0 already followed. Verified on the live DB: all 5 \u03c0-family papers now yield their model token, and the misfiled Q&A re-resolves to the right page |
...
| Linking a citation-dense paragraph fails with a Notion 400 and aborts the whole run | Linking one marker splits a paragraph into `[`, the number, `]` and the prose on either side — three extra spans each. Notion caps a block at **100 rich_text spans**, so a paragraph with 32 markers produced 131 and the PATCH failed; the exception then stopped the run, leaving the earlier chapters written and the later ones untouched. Same limit the assembler already had to respect (`MAX_RICH_TEXT_SPANS`), rediscovered from the other side | The brackets carry no link, so they belong with the prose beside them: `coalesce()` merges adjacent unlinked spans that share formatting, taking that paragraph from 131 to 65. A block that still exceeds the cap raises before the PATCH, so it is reported rather than aborting everything after it |
| An author-year paper gets a reference list and not one working link | `link_references` only understood NUMERIC markers. Roughly half the papers here never cite by number at all — the body says `(Brohan et al. 2022)` and the list is labelled `Brohan et al. (2022)` — so the whole feature was a no-op on them: one page had **73 citations and zero links**. Worse, the numeric path is the one that needs alignment and can be wrong, while this one cannot: the label IS the citation | Index the bibliography by `(name, year)` and link every `Author et al. YEAR` in the body to its entry. No alignment, no ordering assumption, no partial-prefix caution needed — name plus year identifies one entry outright, and the year's `a`/`b` suffix is what separates an author's two papers in the same year, so it belongs in the key. Normalising away punctuation matters: a body writes `O'Neill` where the entry has `O’Neill`. **The label is bracketed in one paper and parenthesised in the next** — accept both, or the index comes out empty and the feature silently does nothing |
...
| A figure appears with no number at all, captioned `(a) Example rollouts.` | A composite figure nests one `<figcaption>` per PANEL and puts its own caption — the one carrying the NUMBER — after them. `parse_figures` took the first match, so the whole figure was captioned with a panel label and its number vanished. One page lost three figure numbers this way, and one of those was additionally MISLABELLED (`A.14` instead of `14`) because `figure_label`'s caption fallback had a panel label to read instead of `Fig. 14:`. Measured across 60 recent pages: **11 affected** | Prefer the `<figcaption>` that opens with a figure label; fall back to the first, which is right for a simple figure. Verified across four papers: every caption now opens with its label. **Repairing an existing page must not re-inject** — that re-places every figure and destroys a hand-arranged layout — so `extract_paper_figures.py --page <id> --recaption` edits captions only, matching each panel caption back to the source figure whose markup contains it |
| Every figure on the page is one number lower than in the paper the reader is holding | The LaTeXML build **omitted the paper's teaser figure**, so its own numbering starts at the second figure and every later number is pulled down by one. Sourcing from arxiv-native HTML does not protect against this — that rule exists for the *stale-v1* trap (ar5iv), which is a different fault. The page stays internally consistent (captions, body references and placement all agree), so nothing flags it; it is only wrong against the PDF. Confirmed by extracting the PDF's own caption for each number and comparing: `PDF n = HTML n-1` all the way down | **The PDF is the authority on figure numbers.** Compare the two before trusting HTML numbering — one command reads each PDF caption and lines it up. To repair: bump every `Fig. N` in the body, archive the figure images, and re-render from the PDF, which supplies the missing teaser too. A figure the PDF's caption scan cannot see can still be sourced from the HTML with its number corrected |
| A figure whose mention sits in plain sight still falls to the end of the document | `_anchor_for` matched `Fig\.`/`Figure` **case-sensitively**, and a translated body writes `fig. 5` and `[fig. 6]` as often as the capitalised form. Those mentions were invisible, so the figure took the end-of-body fallback while the text citing it sat chapters earlier. Same family as the Korean-particle boundary bug, and it hid behind it: both make a real mention unmatchable | `re.I`. Two figures moved straight from the end of the document into the subsections that cite them |
...
| The resolver answers "which paper?" for a message that names its paper, while the retroactive backstop attributes the SAME message correctly | Two tierings, one question. `resolve_paper.py` scores arxiv id -> title keywords -> pasted-body grep and never gained the naming evidence the backstop had learned, so a name — the acronym or model number a title opens with — was read on one path and invisible on the other. The failure direction is safe (it asks rather than writing to the wrong page), but a reader who named the paper precisely is asked to name it again. The backstop's own copy also did not do what its comment claimed: it keyed names by `nm.lower()` and matched with `re.I`, so the case-SENSITIVE acronym rule it documents was never in force and a lowercase English word could resolve on it | One definition both paths import: `auto_save_qa.named_paper(text, papers)`. The resolver calls it ABOVE its keyword tier (naming is evidence, a score is a guess) and BELOW the id tier (an explicit id is the one signal stronger than a name), reporting `how = name`. Case-sensitivity is now real — a digit-carrying model number matches in any case, a pure acronym only in capitals. The page-body test is injectable, so the shared rule is unit-tested with plain dicts and no Notion access, and it now runs only for a name the text actually contains instead of fetching a body per candidate on every call |
